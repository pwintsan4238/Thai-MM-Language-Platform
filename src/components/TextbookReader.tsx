...
export const TextbookReader: React.FC<TextbookReaderProps> = ({ bookId, onClose }) => {
  ...
  const [vocabPage, setVocabPage] = useState<number>(1);
  ...
  const getBookDetails = () => {
    switch (bookId) {
      ...
      default: {
        let title = "Study Companion textbook";
        let myanmarTitle = "နောက်ဆက်တွဲ လေ့ကျင့်ခန်းစာအုပ်";
        const saved = localStorage.getItem('thai_store_items_list');
        if (saved) {
          try {
            const list = JSON.parse(saved);
            const found = list.find((item: any) => item.id === bookId);
            if (found) {
              title = found.name;
              myanmarTitle = found.nameMm || found.name;
            }
          } catch (e) {}
        }
        return {
          ...
          {activeTab === 'page13' ? (
                  ...
                  <div className="relative h-[495px] w-[500px] mx-auto select-none overflow-visible">
                    {sentencePage === 1 ? (
                      ...
                      <>
                        ...
                        {/* SINGLE ANCHOR RIGHT */}
                        <div className="absolute left-[325px] top-[210px] z-30">
                          <div 
                            onClick={() => triggerTTS("ทำ")}
                            className="w-[170px] h-[75px] bg-[#FAF8F5] border-2 border-stone-600/80 rounded-2xl flex flex-col items-center justify-center text-[11px] font-black tracking-wide cursor-pointer hover:bg-[#6366F1] hover:text-white hover:border-[#6366F1] hover:scale-105 active:scale-95 transition-all shadow-xs text-stone-900 leading-tight"
                            title="Click to play: ทำ (လုပ်)"
                          >
                            <span className="text-[10px] opacity-70 font-sans tracking-tight mb-0.5">all verbs +</span>
                            <span className="text-[14px] font-black">ทำ</span>
                            <span className="text-[11px] opacity-95 font-sans mt-0.5">လုပ်</span>
                          </div>
                        </div>
            ...
            <div className="flex-1 flex flex-col justify-between animate-fade-in space-y-4 px-1 pb-4">
              ...
              {(() => {
                ...
                const items = (customRes && customRes.vocabEntries && customRes.vocabEntries.length > 0)
                  ? customRes.vocabEntries
                  : (bookId === 'premium-book' ? PREMIUM_BOOK_VOCAB : SAYAR_SON_JAI_VOCAB);
                ...
                const currentPage = Math.min(Math.max(1, vocabPage), totalPages);
                ...
                return (
                  <>
                    ...
                    {/* Pagination Bar */}
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between pt-2 border-t border-slate-100 select-none px-1">
                        <button
                          type="button"
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-sans font-black transition-all cursor-pointer ${
                            currentPage === 1
                              ...
                              : 'text-brand-purple hover:bg-brand-purple-light hover:scale-105'
                          }`}
                        >
                          <ChevronLeft className="w-4 h-4" />
                        ...
                        <button
                          type="button"
                          onClick={() => setVocabPage(prev => Math.min(totalPages, prev + 1))}
            ...
            <div className="flex-1 flex flex-col justify-between animate-fade-in space-y-4 px-1 pb-4">
              ...
              {(() => {
                const customRes = getCustomResourceData();
                const qaList = (customRes && customRes.dialogueEntries && customRes.dialogueEntries.length > 0)
                  ? customRes.dialogueEntries
                  : (bookId === 'premium-book' ? PREMIUM_BOOK_QA : SAYAR_SON_JAI_QA);
                ...
                const totalPages = Math.ceil(filteredQa.length / ITEMS_PER_PAGE) || 1;
                const currentPage = Math.min(Math.max(1, qaPage), totalPages);
                const displayed = filteredQa.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
                  ...
                  <>
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between pt-2 border-t border-slate-100 select-none px-1">
                        <button
                          type="button"
                          onClick={() => setQaPage(prev => Math.max(1, prev - 1))}
                          disabled={currentPage === 1}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-sans font-black transition-all cursor-pointer ${
                            currentPage === 1
                              ? 'opacity-30 cursor-not-allowed text-slate-400'
                        ...
                        <button
                          type="button"
                          onClick={() => setQaPage(prev => Math.min(totalPages, prev + 1))}
                          disabled={currentPage === totalPages}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-sans font-black transition-all cursor-pointer ${
                            currentPage === totalPages
                              ? 'opacity-30 cursor-not-allowed text-slate-400'
            ...
            <div className="flex-1 flex flex-col justify-between animate-fade-in space-y-4 px-1 pb-4">
              ...
              {/* Robust Conversation sentences list with Pagination */}
              {(() => {
                const customRes = getCustomResourceData();
                const convList = (customRes && customRes.conversationEntries && customRes.conversationEntries.length > 0)
                  ? customRes.conversationEntries
                  : (bookId === 'premium-book' ? PREMIUM_BOOK_CONVERSATION : SAYAR_SON_JAI_CONVERSATION);
                ...
                const ITEMS_PER_PAGE = 5;
                const totalPages = Math.ceil(filteredConv.length / ITEMS_PER_PAGE) || 1;
                const currentPage = Math.min(Math.max(1, conversationPage), totalPages);
                const displayedConv = filteredConv.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);

                return (
                  <>
                    <div className="flex-grow overflow-y-auto max-h-[340px] space-y-3.5 pr-1">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-100 select-none px-1">
                        <button
                          ...
                          className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black transition-all cursor-pointer ${
                            currentPage === 1
                              ? 'opacity-30 cursor-not-allowed text-slate-400'
                              : 'text-[#4f46e5] hover:bg-[#4f46e5]/5 hover:scale-105'
                          }`}
                        >
                        ...
                        <div className="flex items-center gap-1">
                          {Array.from({ length: totalPages }).map((_, i) => {
                            ...
                            if (!isNear && !isEdge) {
                              if (pageNum === 2 || pageNum === totalPages - 1) {
                                return <span key={pageNum} className="text-[10px] text-slate-400 font-black px-0.5">...</span>;
                              }
                              return null;
                            }
                            return (
                              <button
                                key={pageNum}
                                type="button"
                                onClick={() => setConversationPage(pageNum)}
                                className={`w-5 h-5 rounded-full text-[9px] font-sans font-black flex items-center justify-center transition-all cursor-pointer ${
                                  currentPage === pageNum
                        ...
                        <button
                          type="button"
                          onClick={() => setConversationPage(prev => Math.min(totalPages, prev + 1))}
                          disabled={currentPage === totalPages}
                          className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black transition-all cursor-pointer ${
                            currentPage === totalPages
                              ? 'opacity-30 cursor-not-allowed text-slate-400'
